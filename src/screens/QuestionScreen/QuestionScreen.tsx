...
import { QUIZ, Subject } from '../../../assets/data/subject'
...
const QuestionScreen = ({ route, navigation }: RootStackScreenProps<'QuestionScreen'>) => {
    ...
    const [currentQuiz, setCurrentQuiz] = useState<QUIZ>(selectedQuestion.quiz[0])
        ...
        setSelectedAnswers(currentSelectedAnswers => {
            if (currentSelectedAnswers.includes(choice)) {
              return currentSelectedAnswers.filter(answer => answer !== choice)
                ...
                if (currentQuiz.CorrectAnswer && currentQuiz.CorrectAnswer.length > 1)
  ...
  const onSubmit = () => {
    ...
    if (selectedAnswers.length !== currentQuiz.CorrectAnswer?.length) {
